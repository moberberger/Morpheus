...
/// <summary>
/// The collection of methods provided by Morpheus on top of any
/// <see cref="Random"/> or <see cref="Rng"/> implementation.
/// </summary>
public static class RandomExtensions
{
    ...
    /// The standard deviation governing the shape of the normal distribution
    /// curve
    /// </param>
    /// <returns>
    /// A random floating-point (double) value that represents a Normal
    /// distribution around _mean with a standard deviation of
    /// _standardDeviation
    /// </returns>
    /// <remarks>
    /// <para> Finds a random point (r1,r2) that is within the unit circle,
    /// excluding the origin. See pp. 312-314 in "Numerical Recipes in C (2nd
    /// Ed.)" entitled "Normal (Gaussian) Deviates". </para> <para> Roughly
    /// 68.269% of values will be +/- ONE _standardDeviation of _mean </para>
    /// <para> Roughly 95.450% of values will be +/- TWO _standardDeviations of
    /// _mean </para> <para> Roughly 99.730% of values will be +/- THREE
    /// _standardDeviations of _mean </para> <para> Roughly 99.994% of values
    /// will be +/- FOUR _standardDeviations of _mean </para>
    /// </remarks>
    ...
    /// <summary>
    /// Returns a random floating-point (double) value that represents a Normal
    /// distribution about zero with a standard deviation of 1
    /// </summary>
    ...
    /// <summary>
    /// Simulate the rolling of a number of dice, where each die has the same
    /// number of sides.
    /// </summary>
    /// <param name="_rng">The Random Number Generator</param>
    /// <param name="_numberOfDice">The number of dice to roll.</param>
    /// <param name="_sidesPerDie">
    /// The number of sides on each die rolled.
    /// </param>
    ...
    /// <summary>
    /// A modulo operation that detects what would be considered "bias" when
    /// scaling a random number from one range to another.
    /// </summary>
    /// <param name="_value">The generated random number</param>
    /// <param name="_exclusiveMax">
    /// returned value will be in [0.._exclusiveMax)
    /// </param>
    /// <param name="_maxValue">
    /// What should be considered the inclusive "Max Value" for the _value
    /// parameter. Two possible reasons for providing this value (and not
    /// accepting the default value of <see cref="ulong.MaxValue"/> ) is (1)
    /// Unit Testing; and (2) Scaling a 32-bit number instead of a 64-bit number
    /// </param>
    /// <param name="_isUnbiased">
    /// When TRUE, there is no bias in the return value. FALSE indicates that
    /// you should probably generate a new random number.
    /// </param>
    ...
    public static ulong ScaleValue( this ulong _value, ulong _exclusiveMax, out bool _isUnbiased )
    {
        ...
        ulong windowCount = ulong.MaxValue / _exclusiveMax;
        ulong maxOKValue = windowCount * _exclusiveMax;

        // If the random bits are small enough to provide a full "window", then
        // the value is unbiased. If it is not, then it falls into that last
        // little partial "window" between [maxOkValue.._maxValue]
    ...
    public static Rng AsSynchronized( this Rng rng ) => new SynchronizedRng( rng );
