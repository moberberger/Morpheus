...
namespace Morpheus
{
    ...
    /// <see cref="Next(int, int)"/> , but speed is significantly faster. </para>
    /// </remarks>
    public abstract class Rng : Random, IDisposable
    {
        ...
        /// </returns>
        public sealed override int Next() => Next( int.MaxValue );
        ...
        public sealed override int Next( int minValue, int maxValue ) => minValue + Next( maxValue - minValue );
        ...
        /// <summary>
        /// If true, a tiny bias is introduced when scaling numbers using <see cref="Next()"/> ,
        /// <see cref="Next(int)"/> and <see cref="Next(int, int)"/> , but speed is
        /// significantly faster.
        /// </summary>
        public bool UseFastScale { get; set; } = false;

        /// <summary>
        /// Return a <see cref="int"/> scaled in [0..maxValue). Checks
        /// <see cref="UseFastScale"/> to see how to perform the scaling operation.
        /// </summary>
        ...
        public override int Next( int maxValue )
        {
            ...
            else
            {
                while (true)
                {
                    int retval = (int)Next64().ScaleValue( (uint)maxValue, uint.MaxValue, out var isUnbiased );
