...
{
    /// <summary>
    /// A collection of various sorting algorithms that may be faster than the IntroSort
    ...
    public static class CSortingAlgorithms
    {
        ...
        /// sort. Uses the default comparator for objects of type T
        /// </summary>
        ...
        /// Selects a member in each TList item to use with the DEFAULT COMPARER
        /// </param>
        ...
        /// <summary>
        /// The QuickSelect algorithm- Split an array such that all items to the "left" of the
        /// position requested are "less than" the position, and all items to the "right" of the
        /// position are "greater than" the item in that position. Useful for placing the median
        /// of an array at the middle of the array.
        /// </summary>
        ...
        /// The position for which elements to the left are less-than, and the elements to the
        /// right are greater than
        /// </param>
        /// <param name="_comparer">
        /// The comparer to use to compare the elements to each other
        /// </param>
        public static void QuickSelect<T>( this T[] _array, int _low, int _high, int _position, IComparer<T> _comparer )
        {
            ...
            // For length = {1,2,3}, simply sort the array fragment using optimized techniques.
            // This is guaranteed to make "position" correct.
            ...
            {
                if (bad12)
                    _array.SwapElements( _low, _low + 1 );
                return;
            }
            ...
            if (len == 3)
            {
                ...
                {
                    _array.SwapElements( _low, _low + 2 );
                    return;
                }
                ...
                if (!bad13)
                {
                    if (bad12)
                        ...
                        _array.SwapElements( _low + 1, _low + 2 );
                }
                else
                {
                    if (bad12)
            ...
            // There are 4 or more, so try to pick a pivot by looking at the middle value from
            // _low, _high and mid
            if (len > 4)
            ...
            if (pivotIdx != _high)
                _array.SwapElements( _high, pivotIdx ); // Now the pivot element is in [_high]
            ...
            for (var i = idx + 1; i < _high; i++)
            {
                ...
                {
                    _array.SwapElements( idx, i );
                    idx++;
                }
            }

            // Now that we've gone through all the items,
            _array.SwapElements( _high, idx ); // swap pivot back into position- idx points to first higher item than pivot
