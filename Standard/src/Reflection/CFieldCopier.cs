...
public class CFieldCopier
{
    ...
    public static Dictionary<string, FieldInfo> GetFieldLookup( Type _type )
    {
        ...
        if (_type.BaseType != typeof( object ) && _type.BaseType is not null)
        {
        ...
        foreach (var fi in
            _type.GetFields( BindingFlags.DeclaredOnly | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance ))
        {
            retval[fi.Name] = fi;
        }
    ...
    {
        var retval = Activator.CreateInstance<TObject>() ?? throw new NullReferenceException( "Didn't Create Instance" );
    ...
    {
        var retval = Activator.CreateInstance( _source.GetType() ) ?? throw new NullReferenceException( "Didn't Create Instance" );
        CopyFields( retval, _source );
        return retval;
    }
}