...
/// </summary>
/// <remarks>
/// This static class contains methods and events that allow a console application to handle
/// process signals, like CTRL-C. It does so by exposing a series of static events that
/// applications can attach "signal handlers" to. <p> If a signal doesn't have a handler
/// attached to it, the signal will be passed back to the OS. If a signal does have a
/// handler, it will NOT be passed back to the OS. For instance, if the application
/// registers an <see cref="OnBreak"/> handler, but no <see cref="OnControlC"/> handler, any
/// "break" events will be handled by the registered handler, but the "ControlC"'s will
/// cause the process to abort. </p> <p> If the <see cref="OnAnySignal"/> event has
/// registered handlers, then ALL signals will be considered "handled" and the OS will not
/// process any of them. </p> <p> The <see cref="IgnoreControlC"/> (with corresponding
/// <see cref="ProcessControlC"/> ) methods are convenient ways to simply ignore the signal
/// generated when the user presses CRTL-C while the application is running. No signals are
/// actually generated for <see cref="OnControlC"/> when the <b> IgnoreControlC </b> method
/// has been called. </p>
/// </remarks>
public static class SignalProcessor
{
    ...
    /// <summary>
    /// Handler that will multiplex a signal (based on ESignalTypes) however the application
    /// wishes.
    /// </summary>
    ...
    /// <summary>
    /// Keep a static reference to the delegate used as a handler to the Win32 call. Must
    /// keep a reference to this delegate around, or the GC will collect it and the Win32
    /// handler will fail.
    /// </summary>
    ...
    /// </summary>
    /// <remarks>
    /// This event gets signalled AFTER all other more specific signals get raised. The
    /// delegate receives information as a parameter detailing what signal raised the event.
    /// If this event has delegates registered with it, all signals will be considered
    /// "handled" and never passed through to the OS.
    /// </remarks>
    ...
    /// <summary>
    /// The process will ignore control-C processing regardless of registered signal
    /// handlers for CTRL-C
    /// </summary>
    ...
    /// <summary>
    /// The process will abort when control-C is pressed and a corresponding signal handler
    /// is not present.
    /// </summary>
